...
function recommend_transform(json_str){
	var language = JSON.parse(json_str)["language_data"]
	console.log(language)
	...
	document.getElementById("recommend_submit_song_type_three").innerHTML = language["recommend_submit"]["song_type"]["three"];
	...
	var i = 1;
	while (i <= recommend_count){
		document.getElementById("recommend_view_download"+i).innerHTML = language["recommend_view"]["download"];
		i = i + 1;
	}
}
